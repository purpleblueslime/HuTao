...
export default function Page() {
  ...
  const callSearch = async () => {
    const albums = await search(se, type);
    $albums(albums);
  };
  ...
  return (
    <div id="app">
      <div className="boxes">
        ...
        <div className="buttons">
          ...
          {d.length <= 0 ? (
            ...
            <button className="button" onClick={rem}>
              rem
            </button>
          )}
        </div>
      ...
      <div className="search" onDrop={() => dropBack()} onDragOver={allowDrop}>
        <div id="bar">
          <input
            className="bar"
            placeholder="Search"
            onKeyUp={(o) =>
              o.keyCode === 13 ? callSearch() : onChange(o.target.value)
            }
          />
          <a className="button" onClick={() => callSearch()}>
            {'>'}
          </a>
        </div>
        <div className="tags">
          <button
            className="tag"
            onClick={() => $type('ANIME')}
            style={type !== 'ANIME' ? { backgroundColor: 'black' } : {}}
          >
            anime
          </button>
          <button
